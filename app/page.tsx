...
<main className={styles.container} >
    ...
    <tbody>
        {Array(Math.ceil((days.length + shift) / 7)).fill(null).map((_, weekIndex) => (
            <tr key={weekIndex}>
                {Array(7).fill(null).map((_, dayIndex) => {
                    ...
                    if (dayNumber < 1 || dayNumber > days.length) {
                        ...
                        return <td className={styles.td} key={dayIndex}>
                          <button onClick={() => openModal(dayNumber)}>
                          {dayNumber}
                          </button>
                          {open && <TodoListModal close={() => setOpen(false)} day={day} month={month} year={year}/>}
                          </td>;
                    }
                })}
            </tr>
